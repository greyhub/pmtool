...
function OrgGate({ orgSlug, children }: { orgSlug: string; children: React.ReactNode }) {
  ...
  const { role, canSponsor } = usePermissions(orgSlug);
  ...
  const sidebarItems = [
    ...
    // The organization-wide audit trail is for owners and admins.
    ...(role !== undefined && canSponsor
      ? [{ href: `/${orgSlug}/audit`, label: tNav('audit'), icon: <AuditIcon /> }]
      : []),
