...
### 1.14 Lịch sử thay đổi (audit)

`entity_history` (tenant-scoped): mỗi dòng là một tạo/sửa/xoá — `entityType`, `entityId`, `action`, `actorId` (FK `SET NULL`; **không lưu tên**, đọc từ `users` khi hiển thị nên tài khoản đã ẩn danh hiện tên ẩn danh), `label` (tên lúc đó, để mục đã xoá vẫn nhận ra), `changes` (`[{field, from, to}]`, hoặc `{field, changed:true}` cho trường dài/riêng tư) hoặc `snapshot` (tạo/xoá), `subjectType/subjectId` (để thay đổi người phụ trách/phụ thuộc/từ điển WBS hiện cả ở công việc). **Ghi tự động ở tầng Prisma** (`prisma/history/history.extension.ts`), không sửa từng service: `create`, `update`, `upsert`, `delete`, `updateMany`, `deleteMany` (và `createMany` của `TaskAssignee`) trên các mô hình liệt kê trong `AUDITED` (`history-diff.ts`) — **thêm mô hình vào `AUDITED` là đủ để có lịch sử**. Chỉ ghi các **cột thật** (không ghi quan hệ `include`/`_count`), bỏ cột nhiễu (`orderIndex`, mốc nhắc…) và chỉ lưu "đã đổi" cho trường dài (`bulky`). Thay đổi được **đệm theo yêu cầu** (`RequestContextStore.history`) và `HistoryFlushInterceptor` chỉ ghi **sau khi yêu cầu thành công** (await trước khi trả lời) — yêu cầu lỗi/giao dịch bị huỷ không để lại bản ghi ma. Đọc trạng thái trước và ghi lịch sử qua `PrismaService.audit`: một **PrismaClient riêng với bể kết nối riêng** (4 kết nối, thô, không phạm vi tenant, không ghi lịch sử) — **bắt buộc tách bể**: nếu dùng chung, một giao dịch đang giữ một kết nối lại chờ thêm một kết nối để đọc trạng thái trước, và nhiều giao dịch cùng lúc sẽ chờ nhau đến hết hạn (bế tắc do cạn bể; với VPS nhỏ bể chỉ ~5 kết nối nên sẽ xảy ra ngay). Test tích hợp chạy với bể chỉ 4 kết nối và có test 12 cập nhật công việc đồng thời để giữ chặt điều này. Thay đổi chỉ chạm cột nhiễu (bộ đếm mã công việc, thứ tự kéo thả) không đọc và không ghi. Công việc ngoài yêu cầu (cron) không ghi. Xoá tổ chức xoá luôn lịch sử (cascade), nên bản ghi của chính yêu cầu xoá đó bị bỏ có chủ đích. API: `GET projects/:key/history` (mọi người xem được dự án; dự án riêng tư 404 với người không có quyền) và `GET organizations/:org/audit` (OWNER/ADMIN), phân trang bằng con trỏ `(createdAt,id)`. Web: `features/history` (`HistoryButton`/`HistoryModal` dùng lại ở mọi phần).
...
Lớp lập kế hoạch tuỳ chọn trên cùng bảng `tasks`: `projects.sprintsEnabled` + `estimationUnit` (POINTS|HOURS); `tasks.sprintId` (null = backlog, FK `ON DELETE SET NULL`) + `tasks.storyPoints`; bảng `sprints` (tenant-scoped) với chỉ mục duy nhất một phần `sprints_one_active_per_project` (`WHERE status='ACTIVE'`) để chặn hai sprint chạy cùng lúc ngay ở tầng dữ liệu. Khối lượng kế hoạch/đã xong được **tính khi đọc** từ các task (không lưu), chỉ `committedLoad` (lúc bắt đầu) và `completedLoad` (lúc đóng) được chụp lại để làm cơ sở cho tốc độ (velocity) và báo cáo sau này. Đóng sprint chuyển việc chưa xong trong một giao dịch. Gán việc vào sprint đã đóng hoặc thuộc dự án khác bị từ chối (`assertSprintUsable`). **Burndown:** `sprint_daily_snapshots` (tenant-scoped, duy nhất theo `(sprintId, date)`) lưu tổng khối lượng và phần đã xong cuối mỗi ngày; ghi khi **bắt đầu** sprint (ngày đầu), khi **đóng** (kết quả, trước khi việc dở bị chuyển đi), hằng đêm lúc 23:55 cùng tác vụ chốt báo cáo ngày (`SprintsService.snapshotActive`, gọi từ `ReportsScheduler`), và **trực tiếp** khi xem sprint đang chạy. `buildBurndown` (`burndown-math.ts`, hàm thuần có test) dựng chuỗi: đường lý tưởng giảm đều theo ngày lịch từ `committedLoad`; ngày thiếu bản chốt mang giá trị trước đó và đánh dấu `estimated`; sprint chạy quá hạn được vẽ tiếp; kết luận so với đường lý tưởng có dung sai 10% mức cam kết. `GET projects/:key/sprints/:id/burndown` (mọi vai trò xem được dự án). **Review:** `tasks.sprintAddedAt` ghi thời điểm việc vào sprint (đặt khi tạo/gán, xoá khi rời; việc chuyển tiếp lúc đóng sprint được đặt lại) để nhận biết việc **phát sinh** (`sprintAddedAt > startedAt`). Khi **đóng**, `sprints.outcome` (JSONB) lưu danh sách việc của sprint — trạng thái, khối lượng, người phụ trách chính, cờ phát sinh, nơi chuyển tiếp — vì việc dở sắp rời sprint; `sprints.goalResult` (MET/PARTIAL/MISSED) và `reviewNotes` do PM+ ghi ở buổi review. `GET/PUT sprints/:id/review`: sprint đang chạy trả bản **xem trước** dựng từ trạng thái hiện tại; đã đóng đọc `outcome`; đã đóng trước khi có tính năng (`outcome` null) trả `detailsAvailable: false` và chỉ dùng `committedLoad/completedLoad`. Phần tính toán thuần ở `review-math.ts` (tóm tắt, theo người, tốc độ trung bình 3 sprint trước). Module: `apps/api/src/modules/sprints`; giao diện: `apps/web/src/features/sprints` (`sprint-review-view.tsx`, `review-text.ts`, `velocity-chart.tsx`).
