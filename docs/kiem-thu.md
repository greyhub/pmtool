...
|---|---|---|---|
| Unit — API | Vitest (Prisma mock) | 224 test / 33 file | ✅ đạt |
| Unit — Web | Vitest + jsdom | 23 test | ✅ đạt |
| Unit — UI (design system, Gantt) | Vitest + Testing Library | 52 test | ✅ đạt |
| Tích hợp — API qua HTTP thật | Supertest + Postgres ephemeral (Testcontainers), toàn bộ guard/interceptor thật | 51 test | ✅ đạt (đã thấy 1 lần một test tên-nhân-vật-đã-dùng lỗi ngẫu nhiên, chạy lại 9 lần liên tiếp đều đạt — chưa tái hiện được, đang theo dõi) |
| End-to-end — trình duyệt thật | Playwright (Chromium), build dev thật của web + api | 30 test / 22 file | ✅ 29 đạt, 1 tự bỏ qua vì chưa cấu hình Telegram trong môi trường này, 1,8 phút |
