...
|---|---|---|---|
| Unit — API | Vitest (Prisma mock) | 265 test / 39 file | ✅ đạt |
| Unit — Web | Vitest + jsdom | 43 test | ✅ đạt |
| Unit — UI (design system, Gantt) | Vitest + Testing Library | 52 test | ✅ đạt |
| Tích hợp — API qua HTTP thật | Supertest + Postgres ephemeral (Testcontainers), toàn bộ guard/interceptor thật | 75 test | ✅ đạt (đã thấy 1 lần một test tên-nhân-vật-đã-dùng lỗi ngẫu nhiên, chạy lại 9 lần liên tiếp đều đạt — chưa tái hiện được, đang theo dõi) |
| End-to-end — trình duyệt thật | Playwright (Chromium), build dev thật của web + api | 42 test / 33 file | ✅ 41 đạt, 1 tự bỏ qua vì chưa cấu hình Telegram trong môi trường này, 1,8 phút |
...
| J4 Thực thi | E2E `task-ux`, `kanban-drag`, `sprints`, `sprint-burndown`, `gamification`; integration "Task history", "Quests", "Task assignees", "Sprints", "Sprint burndown" | Telegram thật không kiểm thử E2E (cần webhook công khai) |
